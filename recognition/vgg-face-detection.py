...
if __name__ == '__main__':
    # file_path = input("images_crop")

    person = 'pig3'
    person_img = 'pig3.jpg'

    img = cv2.imread("./images_orig/" + person + "/" + person_img)
    if img is None or img.size == 0:
        ...
        for (i, rect) in enumerate(rects):
            ...
            cv2.imwrite(os.getcwd() + '/images_crop/' + person + '/' + person_img, img_crop)
            ...
            img = cv2.putText(img, str(np.max(person)), (right, bottom + 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
