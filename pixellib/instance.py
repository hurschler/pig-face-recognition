...
class custom_segmentation:
    ...
    def segmentImage(self, image_file_name, image_path, show_bboxes = False,mask_points_values = False, process_frame = False, output_image_name = None, verbose = None):
        if process_frame ==False:
            image = cv2.imread(image_path)
        ...
        if show_bboxes == False:
            ...
            if mask_points_values == False:
              output = display_box_instances(image_file_name, image, r['rois'], r['masks'], r['class_ids'], self.config.class_names, r['scores'])
...
def apply_mask(image, mask, color, alpha=0.5):
    ...
    for c in range(3):
        ...
        # Damit alles andere eingefärbt wird (weisser hintergrund, farbiges Säuli)
        # image[:, :, c] = np.where(mask != 1, image[:, :, c] * (1 - alpha) + alpha * color[c] * 255, image[:, :, c])
...
def display_box_instances(image_file_name, image, boxes, masks, class_ids, class_name, scores):
    ...
    for i, color in enumerate(colors):
        ...
        # Keep only images with a Face detection Score more then x
        # if score < 0.90:
        #     continue
        ...
        # cv2.imwrite('image_befor_bitwise_and' + str(i) + '.jpg', image)
        output = cv2.bitwise_and(image, image, mask = outputMask)

        # cv2.imwrite('image_after_bitwise_and' + str(i) + '.jpg', output)
